...
def draw(code,tl=(15,2),font_size=23,img_size=(100,30),bg=(255,255,255),fg=(0,0,255),mode='RGB'):
    ...
    painter.text(tl,code,font=font,fill=fg)
    #img=img.rotate(-60)
    return img

#def rotate(img):
    #rot = img.rotate(random.randint(-10,10),expand=0) #默认为0，表示剪裁掉伸到画板外面的部分
    #fff = Image.new('RGBA',rot.size,(255,)*4)
    #img = Image.composite(rot,fff,rot)
    #return img

def create_lines(img,n_line=(1, 4)):
    '''绘制干扰线'''
...
def create_points(img,point_chance=5):
    '''绘制干扰点'''
